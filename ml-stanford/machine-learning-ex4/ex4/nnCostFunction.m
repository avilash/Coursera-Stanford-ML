...
delta_3 = OL - ymod;
Theta2_grad = Theta2_grad + delta_3'*HL;
delta_2 = (delta_3*Theta2).*(HL.*(1-HL));
...
for iter = 1:m
%J = J + (1/m)*(((-ymod(iter,:))*log(OL(iter,:)'))-((1-ymod(iter,:))*log((1-OL(iter,:))')));
h_vec = OL(iter,:);
y_vec = ymod(iter,:);
J = J + (1/m)*(((-y_vec)*log(h_vec'))-((1-y_vec)*log((1-h_vec)')));
end

Reg_Term = sum((Theta1(:,2:end).^2)(:)) + sum((Theta2(:,2:end).^2)(:));
